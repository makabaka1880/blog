<template>
    <div class="audio-player-container box-container">
        ...
        <div class="audio-player">
            <button class="play-pause-toggle" type="button" :disabled="!audioSrc" :aria-pressed="isPlaying"
                :aria-label="isPlaying ? 'Pause' : 'Play'" @click="togglePlay">
            ...
            <div class="volume-wrapper">
                <button class="volume-button" type="button" :aria-label="isMuted ? 'Unmute' : 'Mute'"
                    @click="toggleMute">
                    <UIKitSvgIcon class="volume-icon" name="volume" :class="{ 'text-muted': isMuted }" />
                </button>
                <div class="volume-popover">
                    <input ref="volumeRange" class="volume-range" type="range" min="0" max="1" step="0.01"
                        v-model.number="volume" @mouseup="releaseFocus" @touchend="releaseFocus" />
                </div>
            </div>

            <audio ref="audio" :src="audioSrc" preload="metadata" @timeupdate="updateTime" @play="isPlaying = true"
                @pause="isPlaying = false" @ended="isPlaying = false" @loadedmetadata="onLoadedMetadata" />
        </div>
    </div>
</template>
